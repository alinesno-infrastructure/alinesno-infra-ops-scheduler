...
public abstract class AbstractMonitor implements Monitor {
    ...
    private void restartServer(String path,Integer port,String installPath) throws Exception{
        ...
        for (String node : needRestartServer){
            // os.system('ssh -p ' + ssh_port + ' ' + self.get_ip_by_hostname(master) + ' sh ' + install_path + '/bin/ops-scheduler-daemon.sh start master-server')
            String runCmd = "ssh -p " + port + " " +  node + " sh "  + installPath + "/bin/ops-scheduler-daemon.sh start " + serverName;
