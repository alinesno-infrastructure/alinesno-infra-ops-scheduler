...
<template>
  <div class="list-model" style="position: relative;">
    <div class="table-box">
      <el-table class="fixed" :data="list" size="small" style="width: 100%" @selection-change="_arrDelChange">
        ...
        <el-table-column :label="$t('State')" width="50" align="center">
          <template slot-scope="scope">
            <span v-html="_rtState(scope.row.state)" style="cursor: pointer;"></span>
          </template>
        </el-table-column>
        <el-table-column :label="$t('Run Type')" align="center">
          <template slot-scope="scope">
            {{_rtRunningType(scope.row.commandType)}}
          </template>
        </el-table-column>
        <el-table-column :label="$t('Scheduling Time')" width="145" align="center">
          <template slot-scope="scope">
            <span v-if="scope.row.scheduleTime">{{scope.row.scheduleTime | formatDate}}</span>
            <span v-else>-</span>
          </template>
        </el-table-column>
        <el-table-column :label="$t('Start Time')" width="145" align="center">
          <template slot-scope="scope">
            <span>{{scope.row.startTime | formatDate}}</span>
          </template>
        </el-table-column>
        <el-table-column :label="$t('End Time')" width="145" align="center">
          <template slot-scope="scope">
        ...
        <el-table-column prop="executorName" :label="$t('Executor')" :show-overflow-tooltip="true"></el-table-column>
        <el-table-column prop="host" :label="$t('host')" min-width="210"></el-table-column>
        <el-table-column :label="$t('Operation')" width="300" align="center" fixed="right">
          <template slot-scope="scope">
            <div v-show="scope.row.disabled">
              <el-tooltip :content="$t('Edit')" placement="top" :enterable="false">
                <span>
                  <el-button type="primary" circle size="small" icon="el-icon-edit-outline" :disabled="scope.row.state !== 'SUCCESS' && scope.row.state !== 'PAUSE' && scope.row.state !== 'FAILURE' && scope.row.state !== 'STOP'" @click="_reEdit(scope.row)"  ></el-button>
                </span>
              </el-tooltip>
              <el-tooltip :content="$t('Rerun')" placement="top" :enterable="false">
                <span><el-button type="primary" circle size="small" :disabled="scope.row.state !== 'SUCCESS' && scope.row.state !== 'PAUSE' && scope.row.state !== 'FAILURE' && scope.row.state !== 'STOP'"  icon="el-icon-refresh" @click="_reRun(scope.row,scope.$index)"  ></el-button></span>
              </el-tooltip>
              <el-tooltip :content="$t('Recovery Failed')" placement="top" :enterable="false">
                <span>
                  <el-button type="success" size="small" circle icon="el-icon-circle-close" :disabled="scope.row.state !== 'FAILURE'" @click="_restore(scope.row,scope.$index)"  ></el-button>
                </span>
              </el-tooltip>
              <el-tooltip :content="scope.row.state === 'STOP' ? $t('Recovery Suspend') : $t('Stop')" placement="top" :enterable="false">
                <span><el-button type="danger" size="small" circle :disabled="scope.row.state !== 'RUNNING_EXECUTION' && scope.row.state !== 'STOP'"  :icon="scope.row.state === 'STOP' ? 'el-icon-video-play' : 'el-icon-close'" @click="_stop(scope.row,scope.$index)"  ></el-button></span>
              </el-tooltip>
              <el-tooltip :content="scope.row.state === 'PAUSE' ? $t('Recovery Suspend') : $t('Pause')" placement="top" :enterable="false">
                <span><el-button type="warning" size="small" circle :icon="scope.row.state === 'PAUSE' ? 'el-icon-video-play' : 'el-icon-video-pause'" :disabled="scope.row.state !== 'RUNNING_EXECUTION' && scope.row.state !== 'PAUSE'" @click="_suspend(scope.row,scope.$index)"  ></el-button></span>
              </el-tooltip>
              <el-tooltip :content="$t('Delete')" placement="top" :enterable="false">
                <el-popconfirm
                  ...
                  <el-button type="danger" size="small" circle icon="el-icon-delete" :disabled="scope.row.state !== 'SUCCESS' && scope.row.state !== 'FAILURE' && scope.row.state !== 'STOP' && scope.row.state !== 'PAUSE'" slot="reference"></el-button>
                </el-popconfirm>
              </el-tooltip>
              <el-tooltip :content="$t('Gantt')" placement="top" :enterable="false">
                <span><el-button type="primary" circle size="small" icon="el-icon-s-operation" @click="_gantt(scope.row)"  ></el-button></span>
              </el-tooltip>
            </div>
            <div v-show="!scope.row.disabled">
              <!--Edit-->
    ...
    <el-tooltip :content="$t('Delete')" placement="top" :enterable="false">
      <el-popconfirm
        ...
        @confirm="_delete({},-1)"
      >
        <el-button style="position: absolute; bottom: -48px; left: 19px;"  type="primary" size="small" :disabled="!strDelete" slot="reference">{{$t('Delete')}}</el-button>
      </el-popconfirm>
    </el-tooltip>
  </div>
</template>
