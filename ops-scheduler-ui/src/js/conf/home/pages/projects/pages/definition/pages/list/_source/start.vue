...
<template>
  <div class="start-process-model">
      ...
      <div class="cont">
        <el-select
          ...
          <el-input slot="trigger" slot-scope="{ selectedModel }" readonly size="small" :value="selectedModel ? selectedModel.label : ''" style="width: 200px;" @on-click-icon.stop="warningGroupId = ''">
            ...
            <em slot="suffix" class="el-icon-bottom" style="font-size: 13px;" v-show="!warningGroupId"></em>
          </el-input>
          <el-option
    ...
    <div class="submit">
      ...
      <el-button type="primary" size="small" :loading="spinnerLoading" @click="ok()">{{spinnerLoading ? $t('Loading...') : $t('Start')}} </el-button>
    </div>
  </div>
</template>
<script>
    ...
    methods: {
      ...
      _showParallelismInfo () {
        this.$message.warning({
          ...
          message: `<p style='font-size: 13px;'>${i18n.$t('Parallelism tip')}</p>`
        })
      },
      _onLocalParams (a) {
        this.udpList = a
