...
<template>
  <div class="list-model" style="position: relative;">
    <div class="table-box">
      <el-table :data="list" size="small" style="width: 100%" @selection-change="_arrDelChange">
        <el-table-column type="selection" width="50" align="center" :selectable="selectable"></el-table-column>
        <el-table-column prop="id" :label="$t('#')"  align="center" width="50"></el-table-column>
        ...
        <el-table-column :label="$t('Create Time')" width="150">
          <template slot-scope="scope">
            <span>{{scope.row.createTime | formatDate}}</span>
          </template>
        </el-table-column>
        <el-table-column :label="$t('Update Time')" width="150">
          <template slot-scope="scope">
        ...
        <el-table-column prop="userName" :label="$t('Create User')" :show-overflow-tooltip="true"></el-table-column>
        <el-table-column prop="modifyBy" :label="$t('Modify User')" :show-overflow-tooltip="true"></el-table-column>

        <el-table-column :label="$t('Timing state')" align="center">
          <template slot-scope="scope">
        ...
        <el-table-column :label="$t('Operation')" width="400" align="center" fixed="right">
          <template slot-scope="scope">
            <el-tooltip :content="$t('Edit')" placement="top" :enterable="false">
              <span><el-button type="primary" size="small" circle icon="el-icon-edit-outline" :disabled="scope.row.releaseState === 'ONLINE'" @click="_edit(scope.row)"  ></el-button></span>
            </el-tooltip>
            <el-tooltip :content="$t('Start')" placement="top" :enterable="false">
              <span><el-button type="success" size="small" circle :disabled="scope.row.releaseState !== 'ONLINE'"  icon="el-icon-video-play" @click="_start(scope.row)"  ></el-button></span>
            </el-tooltip>
            <el-tooltip :content="$t('Timing')" placement="top" :enterable="false">
              <span><el-button type="primary" size="small" circle icon="el-icon-time" :disabled="scope.row.releaseState !== 'ONLINE' || scope.row.scheduleReleaseState !== null" @click="_timing(scope.row)"  ></el-button></span>
            </el-tooltip>
            <el-tooltip :content="$t('online')" placement="top" :enterable="false">
              <span><el-button type="warning" size="small" circle v-if="scope.row.releaseState === 'OFFLINE'"  icon="el-icon-upload2" @click="_poponline(scope.row)"  ></el-button></span>
            </el-tooltip>
            <el-tooltip :content="$t('offline')" placement="top" :enterable="false">
              <span><el-button type="danger" size="small" circle icon="el-icon-download" v-if="scope.row.releaseState === 'ONLINE'" @click="_downline(scope.row)"  ></el-button></span>
            </el-tooltip>
            <el-tooltip :content="$t('Copy Workflow')" placement="top" :enterable="false">
              <span><el-button type="primary" size="small" circle :disabled="scope.row.releaseState === 'ONLINE'"  icon="el-icon-document-copy" @click="_copyProcess(scope.row)"  ></el-button></span>
            </el-tooltip>
            <el-tooltip :content="$t('Cron Manage')" placement="top" :enterable="false">
              <span><el-button type="primary" size="small" circle icon="el-icon-date" :disabled="scope.row.releaseState !== 'ONLINE'" @click="_timingManage(scope.row)"  ></el-button></span>
            </el-tooltip>
            <el-tooltip :content="$t('Delete')" placement="top" :enterable="false">
              ...
              >
                <el-button type="danger" size="small" circle icon="el-icon-delete" :disabled="scope.row.releaseState === 'ONLINE'" slot="reference"></el-button>
              </el-popconfirm>
            </el-tooltip>
            <el-tooltip :content="$t('TreeView')" placement="top" :enterable="false">
              <span><el-button type="primary" size="small" circle icon="el-icon-s-data" @click="_treeView(scope.row)"  ></el-button></span>
            </el-tooltip>
            <el-tooltip :content="$t('Export')" placement="top" :enterable="false">
              <span><el-button type="primary" size="small" circle icon="el-icon-s-unfold" @click="_export(scope.row)"  ></el-button></span>
            </el-tooltip>
            <el-tooltip :content="$t('Version Info')" placement="top" :enterable="false">
              <span><el-button type="primary" size="small" circle icon="el-icon-info" @click="_version(scope.row)"  ></el-button></span>
            </el-tooltip>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-tooltip :content="$t('Delete')" placement="top">
      <el-popconfirm
        ...
        @confirm="_delete({},-1)"
      >
        <el-button style="position: absolute; bottom: -48px; left: 19px;"  type="primary" size="small" :disabled="!strSelectCodes" slot="reference">{{$t('Delete')}}</el-button>
      </el-popconfirm>
    </el-tooltip>
    <el-button type="primary" size="small" :disabled="!strSelectCodes" style="position: absolute; bottom: -48px; left: 80px;" @click="_batchExport(item)" >{{$t('Export')}}</el-button>
    <span><el-button type="primary" size="small" :disabled="!strSelectCodes" style="position: absolute; bottom: -48px; left: 140px;" @click="_batchCopy(item)" >{{$t('Batch copy')}}</el-button></span>
<!--    <el-button type="primary" size="small" :disabled="!strSelectCodes" style="position: absolute; bottom: -48px; left: 225px;" @click="_batchMove(item)" >{{$t('Batch move')}}</el-button>-->
    <el-drawer
      :visible.sync="drawer"
      size=""
      :with-header="false">
