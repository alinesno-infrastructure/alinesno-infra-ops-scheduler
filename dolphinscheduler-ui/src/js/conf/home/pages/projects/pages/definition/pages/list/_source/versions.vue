...
<template>
  <div class="container">
    ...
    <div class="table-box" v-if="versionData.processDefinitionVersions.length > 0">
      <el-table :data="versionData.processDefinitionVersions" size="small" style="width: 100%">
          ...
          <template slot-scope="scope">
            <el-tooltip :content="$t('Switch To This Version')" placement="top">
              <el-popconfirm
                ...
                @confirm="_mVersionSwitchProcessDefinitionVersion(scope.row)"
              >
                <el-button :disabled="versionData.processDefinition.releaseState === 'ONLINE' || scope.row.version === versionData.processDefinition.version || isInstance" type="primary" size="small" icon="el-icon-warning" slot="reference"></el-button>
              </el-popconfirm>
            </el-tooltip>
            <el-tooltip :content="$t('Delete')" placement="top">
              <el-popconfirm
                ...
                iconColor="red"
                :title="$t('Delete?')"
                @confirm="_mVersionDeleteProcessDefinitionVersion(scope.row)"
              >
                <el-button :disabled="scope.row.version === versionData.processDefinition.version || isInstance" type="danger" size="small" icon="el-icon-delete" slot="reference"></el-button>
              </el-popconfirm>
            </el-tooltip>
          </template>
        </el-table-column>
    ...
    <div v-if="versionData.processDefinitionVersions.length > 0">
      <div class="bottom-box">
        ...
        <el-button type="text" size="small" @click="_close()" style="float:right">{{$t('Cancel')}}</el-button>
      </div>
    </div>
