...
public class NetUtils {
    ...
    public static String getAddr(String host, int port) {
    	...
    	// 判断优先级，从jvm中获取优先级最高
    	String DS_IP_TO_REGISTRY = System.getProperty("DS_IP_TO_REGISTRY") ; 
    	String DS_PORT_TO_REGISTRY = System.getProperty("DS_PORT_TO_REGISTRY") ; 
    
    	logger.debug("DS_IP_TO_REGISTRY:{} , DS_PORT_TO_REGISTRY:{}" , DS_IP_TO_REGISTRY , DS_PORT_TO_REGISTRY);
    	
    	if(StringUtils.isNotBlank(DS_IP_TO_REGISTRY)) {
    		host = DS_IP_TO_REGISTRY ; 
    	}
    	
    	if(StringUtils.isNotBlank(DS_PORT_TO_REGISTRY)) {
    		port = Integer.parseInt(DS_PORT_TO_REGISTRY) ; 
    	}
